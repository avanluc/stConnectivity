...
// nof_distNodes = DD[s];
    ...
    gpuErrchk( cudaMemcpyToSymbol(devNextLevel, NextLevel, sizeof(bool)*4, 0,cudaMemcpyHostToDevice) );
...
/*############# OLD VERSION ###############*/
    // do{
    // 	stConn1<<< grid, block >>>(Dvertex, Dedges, Ddist_Col, level, N, nof_distNodes, DMatrix);
