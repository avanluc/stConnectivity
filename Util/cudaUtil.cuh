...
namespace cudaUtil {
	...
	//void Compare(T* hostArray, T* devArray, const int size, std::string str = "", bool CHECK = true);
	...
	template<bool SORT, bool FAULT, typename T>
	void Compare(T* hostArray, T* devArray, const int size, std::string str, bool CHECK) {
		if (!CHECK)
			return;
		...
		fUtil::Compare<SORT, FAULT, T>(hostArray, ArrayCMP, size);
